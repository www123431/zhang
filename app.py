...
def get_ebbinghaus_reviews(log_df):
    ...
    temp_df = log_df.copy()
    temp_df['date_dt'] = pd.to_datetime(temp_df['date'], errors='coerce').dt.date
    today = datetime.date.today()
    intervals = [1, 3, 7, 15]
    target_dates = [today - datetime.timedelta(days=i) for i in intervals]
    return temp_df[temp_df['date_dt'].isin(target_dates)]
...
gc = init_connection()
try:
    sh = gc.open("Sheet1") # 需与 Google Sheets 总文件名一致
except:
...
with tab1:
    with st.sidebar:
        st.header("⚙️ 卿姐调频")
        selected_level = st.select_slider("选择难度：", options=["🌱 基础", "✨ 进阶", "💪 核心"])
        st.divider()
        if not clean_log_df.empty:
            csv_data = clean_log_df.to_csv(index=False).encode('utf-8-sig')
            st.download_button("📥 下载唯一词汇足迹", data=csv_data, file_name=f"卿姐学习足迹_{datetime.date.today()}.csv")
    ...
    if st.button("🚀 开启今日助记之旅", use_container_width=True):
        if not lib_df.empty:
            ...
            if not f.empty:
                ...
                st.info("当前难度库中没有匹配单词。")

    if 'batch' in st.session_state:
        for item in st.session_state['batch']:
            word = item.get('word', 'N/A')
            pos_style = get_pos_style(item.get('category', ''))
            st.markdown(f"""
                <div class="word-card">
                    ...
                    <div style="color:#999; font-size:0.85rem; border-top:1px dashed #EEE; padding-top:10px; margin-top:15px;">
                        💡 <b>助记提示：</b> 长度: {len(word)} | 尝试把这个词放进你最近的一个工作场景中。
                    </div>
                </div>
            """, unsafe_allow_html=True)
        
        if st.button("✅ 卿姐记住了，打卡同步", type="primary", use_container_width=True):
            for item in st.session_state['batch']:
                ws_log.append_row([str(datetime.date.today()), item.get('word'), item.get('chinese'), item.get('category'), selected_level])
            st.balloons()
            st.success("🎉 同步成功！明天的复习中心会看到它们。")
            time.sleep(1)
            st.rerun()
...
with tab2:
    st.subheader("🔁 艾宾浩斯复习")
    review_data = get_ebbinghaus_reviews(raw_log_df)
    if not review_data.empty:
        review_data = review_data.drop_duplicates(subset=['word']) # 复习展示也去重
        cols = st.columns(2)
        ...
        st.info("✨ 卿姐，今日暂无复习任务，继续加油！")

# --- Tab 3: 学习足迹 (去重展示) ---
with tab3:
    st.subheader("📚 唯一词汇档案")
    st.caption("这里仅显示每个单词最近一次的学习记录，确保不出现重复。")
    st.dataframe(clean_log_df.drop(columns=['category']) if 'category' in clean_log_df.columns else clean_log_df, use_container_width=True)
